...
function personListRender() {
  ...
  arrayTop3.forEach(function (item, index) {
    ...
    let message = "";
    if (item.haveTen == "是") {
      underTenTag = `<li class="l-badges__item">
            <div class="o-badge">Under 10 mins</div>
        </li>`;
    }
    if (item.codepenUrl !== "") {
      codeTag = `<div class="l-badges__item">
            <div class="o-badge o-badge--outline"><a href ="${item.codepenUrl}">Code</a></div>
        </div>`;
    }
    if (item.youtubeUrl !== "") {
      videoTag = `<div class="l-badges__item">
              <div class="o-badge o-badge--outline"><a href ="${item.codepenUrl}">Code</a></div>
          </div>`;
    }
    if (item.message !== undefined) {
      message = `    <div class="c-card__main">
        <div class="l-stack">
  ...
  arrayOther.forEach(function (item, index) {
    ...
    if (item.message !== undefined) {
      message = `<div class="l-stack__top">
        <p class="o-title o-title--sm o-title--gy">留言</p>
    </div>
    <div class="l-stack__cnt">
        <p class="o-txt">${item.message}</p>
...
function teamCard(team, number) {
  ...
  let text = `<li class="l-billboardList__item">
                    ...
                    <ul class="l-memberList">
                        <li class="l-memberList__item">
                            <div class="c-memberClip">
                                <div class="c-memberClip__title">${
                                  memberFri.slackName
                                }</div>
                                <div class="c-memberClip__timer">${
                                  memberFri.practiceMinute
                                }分${memberFri.practiceSecond}秒</div>
                            </div>
                        </li>
                        <li class="l-memberList__item">
                            <div class="c-memberClip">
                                ...
                                <div class="c-memberClip__timer">${
                                  memberSec.practiceMinute
                                }分${memberSec.practiceSecond}秒</div>
                            </div>
                        </li>
                        <li class="l-memberList__item">
                            <div class="c-memberClip">
                                ...
                                <div class="c-memberClip__timer">${
                                  memberTri.practiceMinute
                                }分${memberTri.practiceSecond}秒</div>
                            </div>
                        </li>
                    </ul>
                </div>
...
function personCard(person, number, underTenTag, codeTag, videoTag, message) {
  return ` <li id="person-${person.slackName}" class="l-card__item">
    <div class="c-card">
        <div class="c-card__info">
            <div class="l-stack l-stack--apart">
                <div class="l-stack__top">
                    <div class="o-badge">No.${number + 3}</div>
                    <p class="o-title o-title--long u-paraL-sm">${
                      person.slackName
                    }</p>
                    <p class="o-txt">${person.practiceMinute}分 ${
    person.practiceSecond
  } 秒</p>
                </div>
                <div class="l-stack__cnt">
                    <ul class="l-badges">
                    ...
                    ${videoTag}
                        <li class="l-badges__item">
                            <div class="o-badge o-badge--gn">TEAM ${
                              person.jsGroup
                            }</div>
                        </li>
                        <li class="l-badges__item">
                            <a  href="#" data-target >
                                <i data-name="${
                                  person.slackName
                                }"  class=" openBtn o-icon o-icon--lg o-icon--more" data-target-ico></i>
                            </a>
                        </li>
                    </ul>
                </div>
...
personOtherList.addEventListener("click", (e) => {
  ...
  let elementClasses = document.querySelector(
    `#person-${dataName} .c-card__panel`
  ).classList;
